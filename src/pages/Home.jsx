...
const Home = () => {
  ...
  useEffect(() => { 
    gettingAllSeriesGenres().then(genre => {
      const excludedGenres = ["Mystery", "Talk", "Sci-Fi & Fantasy"];
  ...
  return (
    <div className='container-fluid'>
      <CorouselComponent trendingSeries={trendingSeries} />
      <div className="trending-section my-3">
        <h2 className='mx-3 my-3'>Trending Now</h2>
      ...
      {genres
        .map((item, i) => (
          <div key={`movies-slider${i}`}>
            <h4 className='mx-3 mt-5'>{item.name}</h4>
            <CardSlider id={item.id} />
          </div>
        ))}
      {seriesGenres.length > 0 && seriesGenres
        .map((item, i) => (
          <div key={`series-slider-${i-100}`}>
            <h4 className='mx-3 my-3'>{item.name}</h4>
